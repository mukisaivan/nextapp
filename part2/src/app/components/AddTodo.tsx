...
export default function AddTodo() {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        // before turning to next screen do this 

        startTransition(() => {
            if (pathname === "/add") { // if we are still on the adding another to list route
                router.push('/')
            } else {
    ...
    const content = (
        <form onSubmit={handleSubmit} className="flex gap-2 items-center" style={{ opacity: !isMutating ? 1 : 0.7 }}>
            ...
            <input
                ...
                className="text-2xl p-1 rounded-lg flex-grow w-full text-red-500"
